...
import { v2 as cloudinary } from "cloudinary"; //to upload here

export const createCourse = async (req, res) => {
  const adminId = req.adminId;  //taki sief admin hi krpaye
  ...
  try {
    ...
    if (!allowedFormat.includes(image.mimetype)) {
      return res
        .status(400)
        .json({ error: " Invalid File Uploaded only jpg and png are allowed" });
    }
    //cloudinary code
    ...
    const courseData = {
      ...
      creatorId: adminId, //taki admin hi krpaye
    };
...
if(!courseSearch){
    ...
    const course = await Course.updateOne(
      {
        _id: courseId,
        creatorId: adminId, //taki sirf admin hi krpaye
      },
      {
        title,
        description,
...
export const deleteCourse = async (req, res) => {
  ...
  try {
    const course = await Course.findOneAndDelete({
      _id: courseId,
      creatorId: adminId, //taki sief admin hi krpaye
    });
    if (!course) {
      return res
        .status(404)
...
export const buyCourses = async (req, res) => {
  ...
  try {
    ...
    res.status(500).json({ errors: "Error in purchasing course" });
